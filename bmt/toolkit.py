...
REMOTE_PATH = (
    "https://raw.githubusercontent.com/biolink/biolink-model/2.2.0/biolink-model.yaml"
)
...
class Toolkit(object):
    ...
    def _get_all_slots_with_class_domain(
        ...
        for k, v in self.generator.schema.slots.items():
            if check_ancestors:
                if (
                    v.domain == element.name
                    or v.domain in self.get_ancestors(element.name, mixin)
